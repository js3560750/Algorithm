...
public class HeapSort {
	 ...
	 * 
	 * 对于节点i，其二叉树左孩子是2*i+1，右孩子是2*i+2，其父节点是(i-1)/2向下取整
	 * 
	 * 堆排序是把待排序序列arr看成完全二叉树的顺序存储结构，大根堆性质：即任意节点不小于其左右两个孩子节点，大根堆的最大元素一定在堆顶
	 * 
	 * 首先执行buildBigHeap函数建立大根堆，建立的过程中从最后一个元素的父节点开始依次减1调用adjustDownToUp函数，因为adjustDownToUp函数是保证其传入的index脚标之后的元素符合大根堆性质。
	 * 因为又是从最后一个元素的父节点开始依次减1调用的adjustDownToUp函数，因此能保证buildBigHeap函数执行完成后，所得序列符合大根堆性质
	 * 
	 * 然后把大根堆的堆顶和最后一位元素互换，作为有序序列的最大值，因为最后一位元素如今变成了堆顶，很可能破坏大根堆性质（任意节点不小于其左右两个孩子节点），因此调用adjustDownToUp函数且传入的index=0，length-1
